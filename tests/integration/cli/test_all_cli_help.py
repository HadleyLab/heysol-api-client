...
def run_help_command(cmd, cwd):
    ...
    try:
        ...
        cmd_list = cmd.split()
        result = subprocess.run(cmd_list, capture_output=True, text=True, cwd=cwd, timeout=10)
        return {
            "command": cmd,
...
class TestAllCLIHelp:
    ...
    def test_main_command_helps(self, cmd, project_root):
        ...
        ), f"Command '{cmd}' failed to show usage: {result['stderr']}"
        assert (
            result["returncode"] == 0
        ), f"Command '{cmd}' exited with non-zero code: {result['returncode']}"
    ...
    def test_memory_subcommand_helps(self, cmd, project_root):
        ...
        assert (
            "Usage:" in result["stdout"]
        ), f"Memory command '{cmd}' failed to show usage: {result['stderr']}"
        assert (
            result["returncode"] == 0
        ), f"Memory command '{cmd}' exited with non-zero code: {result['returncode']}"
    ...
    def test_logs_subcommand_helps(self, cmd, project_root):
        ...
        assert (
            "Usage:" in result["stdout"]
        ), f"Logs command '{cmd}' failed to show usage: {result['stderr']}"
        assert (
            result["returncode"] == 0
        ), f"Logs command '{cmd}' exited with non-zero code: {result['returncode']}"
    ...
    def test_spaces_subcommand_helps(self, cmd, project_root):
        ...
        assert (
            "Usage:" in result["stdout"]
        ), f"Spaces command '{cmd}' failed to show usage: {result['stderr']}"
        assert (
            result["returncode"] == 0
        ), f"Spaces command '{cmd}' exited with non-zero code: {result['returncode']}"

    @pytest.mark.parametrize("cmd", ["./heysol-cli profile get --help"])
    def test_profile_subcommand_helps(self, cmd, project_root):
        ...
        assert (
            "Usage:" in result["stdout"]
        ), f"Profile command '{cmd}' failed to show usage: {result['stderr']}"
        assert (
            result["returncode"] == 0
        ), f"Profile command '{cmd}' exited with non-zero code: {result['returncode']}"
    ...
    def test_registry_subcommand_helps(self, cmd, project_root):
        ...
        assert (
            "Usage:" in result["stdout"]
        ), f"Registry command '{cmd}' failed to show usage: {result['stderr']}"
        assert (
            result["returncode"] == 0
        ), f"Registry command '{cmd}' exited with non-zero code: {result['returncode']}"

    @pytest.mark.parametrize("cmd", ["./heysol-cli tools list --help"])
    def test_tools_subcommand_helps(self, cmd, project_root):
        ...
        assert (
            "Usage:" in result["stdout"]
        ), f"Tools command '{cmd}' failed to show usage: {result['stderr']}"
        assert (
            result["returncode"] == 0
        ), f"Tools command '{cmd}' exited with non-zero code: {result['returncode']}"
    ...
    def test_webhooks_subcommand_helps(self, cmd, project_root):
        ...
        assert (
            "Usage:" in result["stdout"]
        ), f"Webhooks command '{cmd}' failed to show usage: {result['stderr']}"
        assert (
            result["returncode"] == 0
        ), f"Webhooks command '{cmd}' exited with non-zero code: {result['returncode']}"
    ...
    def test_all_help_commands_summary(self, project_root):
        ...
        command_groups = {
            ...
            "profile": ["./heysol-cli profile get --help"],
            "registry": [
                "./heysol-cli registry register --help",
                "./heysol-cli registry list --help",
                "./heysol-cli registry show --help",
            ...
            "tools": ["./heysol-cli tools list --help"],
            "webhooks": [
                "./heysol-cli webhooks create --help",
                "./heysol-cli webhooks list --help",
                "./heysol-cli webhooks get --help",
        ...
        assert (
            failed == 0
        ), f"{failed} out of {total_commands} help commands failed. Check stderr for details."

        # Additional check: Ensure no timeouts or errors
        for result in results:
            assert (
                result["returncode"] == 0
            ), f"Command '{result['command']}' failed with code {result['returncode']}: {result['stderr']}"
...
class TestCLIPolish:
    ...
    def test_command_descriptions_polish(self, project_root):
        ...
        commands = [
            (
                "./heysol-cli memory --help",
                "Memory operations: ingest, search, queue, and episode management",
            ),
            ("./heysol-cli logs --help", "Manage ingestion logs, status, and log operations"),
            (
                "./heysol-cli spaces --help",
                "Space management: create, list, update, delete, and bulk operations",
            ),
            ("./heysol-cli profile --help", "User profile and API health check operations"),
            (
                "./heysol-cli registry --help",
                "Manage registered HeySol instances and authentication",
            ),
            ("./heysol-cli tools --help", "List MCP tools and integrations"),
            (
                "./heysol-cli webhooks --help",
                "Webhook management: create, list, update, delete webhooks",
            ),
        ]
